...
# Custom Datasets
from datasets import CounterfactualWaterbirdsDataset
...
new_dir = "/local/scratch/a/bai116/datasets/waterbirds-cf-clip_v1.0/"
root_dir = "/local/scratch/a/bai116/datasets/"
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
hparam = {"input_shape": (3, 224, 224)}
preprocessor = Clip(hparam).to(device)
dataset = CounterfactualWaterbirdsDataset(root_dir=root_dir, download=False)
